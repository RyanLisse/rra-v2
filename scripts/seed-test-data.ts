...
async function seedDocuments(userIds: string[], preset: SeedPreset) {
  ...
  for (let i = 0; i < preset.documents; i++) {
    ...
    // Create content
    const fullContent = faker.helpers.arrayElements(
    ...
    for (let j = 0; j < preset.chunksPerDocument; j++) {
      ...
      const chunkContent = faker.helpers.arrayElement(SAMPLE_CONTENT[contentType]);
  ...
  for (let i = 0; i < embeddings.length; i += embeddingBatchSize) {
    await db.insert(documentEmbedding).values(embeddings.slice(i, i + embeddingBatchSize));
  }
...
async function main() {
  ...
  console.log(`  - Total chunks: ${preset.documents * preset.chunksPerDocument}`);
  console.log('');
