...
async function readPerformanceResults(path: string): Promise<PerformanceResults> {
...
function compareMetrics(
  ...
  current.forEach(currentMetric => {
    ...
    if (baselineMetric) {
      ...
      const threshold = REGRESSION_THRESHOLDS[currentMetric.name] || REGRESSION_THRESHOLDS.default;
...
function generateDetailedReport(comparisons: ComparisonResult[]): Record<string, any> {
  const report: Record<string, any> = {
    ...
    stable: comparisons.filter(c => !c.regression && c.percentageChange >= -5).length,
    metrics: {}
  };
