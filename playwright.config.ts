...
const getReporters = () => {
  ...
  if (isCI) {
    reporters.push(['junit', { 
      outputFile: path.join(process.env.TEST_METRICS_OUTPUT_DIR || './test-results', 'playwright-junit.xml') 
    }]);
    reporters.push(['json', { 
      outputFile: path.join(process.env.TEST_METRICS_OUTPUT_DIR || './test-results', 'playwright-report.json') 
    }]);
  }
  
  reporters.push(['html', {
    outputFolder: path.join(process.env.TEST_METRICS_OUTPUT_DIR || './test-results', 'playwright-report'),
...
export default defineConfig({
  ...
  fullyParallel: process.env.PLAYWRIGHT_FULLY_PARALLEL !== 'false' && !isNeonEnabled,
  ...
  globalSetup: process.env.PLAYWRIGHT_USE_NEON_GLOBAL_SETUP === 'true' 
    ? require.resolve('./tests/config/playwright-neon-setup.ts')
    : undefined,
  
  /* Enhanced global teardown */
  globalTeardown: process.env.PLAYWRIGHT_USE_NEON_GLOBAL_SETUP === 'true'
    ? require.resolve('./tests/config/playwright-neon-setup.ts')
    : undefined,
  ...
  use: {
    ...
    trace: process.env.PRESERVE_TEST_ARTIFACTS_ON_FAILURE === 'true' 
      ? 'retain-on-failure' 
      : 'off',
    
    /* Video recording for debugging */
    video: process.env.PRESERVE_TEST_ARTIFACTS_ON_FAILURE === 'true' 
      ? 'retain-on-failure' 
      : 'off',
  ...
  outputDir: path.join(process.env.TEST_METRICS_OUTPUT_DIR || './test-results', 'playwright-artifacts'),
    ...
    /* E2E tests project */
    {
      ...
      use: {
        ...
        contextOptions: {
          ...
          recordHar: process.env.ENABLE_REQUEST_LOGGING === 'true' ? {
            path: path.join(process.env.TEST_METRICS_OUTPUT_DIR || './test-results', 'network-logs.har'),
            mode: 'minimal',
          } : undefined,
        },
      },
