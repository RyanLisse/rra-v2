...
export const POST = withAuth(async (request: NextRequest, session: any) => {
  try {
    ...
    if (includePerformanceMetrics && iterations > 1) {
      ...
      const avgV3Time = v3Times.reduce((sum, time) => sum + time, 0) / v3Times.length;
      const avgV4Time = v4Times.reduce((sum, time) => sum + time, 0) / v4Times.length;
      const v3StdDev = Math.sqrt(v3Times.reduce((sum, time) => sum + Math.pow(time - avgV3Time, 2), 0) / v3Times.length);
      const v4StdDev = Math.sqrt(v4Times.reduce((sum, time) => sum + Math.pow(time - avgV4Time, 2), 0) / v4Times.length);
        ...
        performanceAnalysis: {
          ...
          speedComparison: {
            ...
            speedDifferencePercent: Math.round(
              (Math.abs(avgV3Time - avgV4Time) / Math.max(avgV3Time, avgV4Time)) * 100
            ),
          },
        },
        recommendation: generateModelRecommendation(lastComparison, avgV3Time, avgV4Time),
        textAnalysis: {
          length: text.length,
...
function generateModelRecommendation(
  ...
  if (comparison.similarity > 0.95) {
    ...
    score4 += 1;
  } else if (comparison.similarity > 0.90) {
  ...
  const speedDifferencePercent = (speedDifference / Math.max(v3Time, v4Time)) * 100;
  
  if (speedDifferencePercent > 20) {
    if (v3Time < v4Time) {
      factors.push(`v3.0 significantly faster (${Math.round(speedDifferencePercent)}% faster)`);
      score3 += 2;
    } else {
      factors.push(`v4.0 significantly faster (${Math.round(speedDifferencePercent)}% faster)`);
      score4 += 2;
    }
  } else if (speedDifferencePercent > 10) {
    if (v3Time < v4Time) {
      factors.push(`v3.0 moderately faster (${Math.round(speedDifferencePercent)}% faster)`);
      score3 += 1;
    } else {
      factors.push(`v4.0 moderately faster (${Math.round(speedDifferencePercent)}% faster)`);
      score4 += 1;
    }
  } else {
  ...
  if (comparison.dimensions2 > comparison.dimensions1) {
    factors.push(`v4.0 has higher dimensions (${comparison.dimensions2} vs ${comparison.dimensions1})`);
    score4 += 1;
  } else if (comparison.dimensions1 > comparison.dimensions2) {
    factors.push(`v3.0 has higher dimensions (${comparison.dimensions1} vs ${comparison.dimensions2})`);
    score3 += 1;
  } else {
