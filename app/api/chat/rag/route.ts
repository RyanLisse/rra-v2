...
const ragChatSchema = z.object({
  ...
  searchOptions: z
    .object({
      ...
      searchType: z.enum(['vector', 'hybrid', 'context-aware', 'multi-step']).optional(),
...
export const POST = withAuth(async (request: Request, session: any) => {
  try {
    ...
    if (searchResponse.results.length === 0) {
      ...
      if (fallbackResponse.results.length === 0) {
        return Response.json({
          ...
          suggestions: await generateSearchSuggestions(message, session.user.id),
        });
      }
    ...
    const ragContext = {
      ...
      totalSources: new Set(qualityFilteredResults.map((r) => r.documentTitle)).size,
      searchMetadata: {
        ...
        averageQuality: qualityFilteredResults.reduce((sum, r) => sum + r.qualityScore, 0) / qualityFilteredResults.length,
      },
    };
          ...
          async start(controller) {
            try {
              ...
              const metadata = {
                ...
                sources: ragContext.chunks.map((chunk) => chunk.documentTitle),
              };
    ...
    if (includeSourceAnalysis) {
      ragResponse = await addSourceAnalysis(ragResponse, qualityFilteredResults);
    }
  ...
  const enhancedResults = await Promise.all(
    results.map(async (result) => {
      ...
      if (conversationContext.length > 0) {
        ...
        const contextRelevance = queryWords.filter(word =>
...
async function generateMultiStepResponse(
  ...
  try {
    ...
    reasoningSteps.push({
      ...
      evidence: [`Query type: ${queryAnalysis.type}`, `Key concepts: ${queryAnalysis.concepts.join(', ')}`],
    ...
    const evidenceEvaluation = evaluateEvidence(ragContext.chunks, queryAnalysis);
    reasoningSteps.push({
      step: 2,
    ...
    reasoningSteps.push({
      ...
      evidence: ['Combined evidence from multiple sources', 'Applied domain knowledge'],
      conclusion: 'Generated comprehensive response based on available evidence.',
    });
...
async function analyzeQuery(query: string): Promise<{
  ...
  const concepts = query
    .toLowerCase()
    .match(/\b[a-z]{3,}\b/g)
    ?.filter(word => !['the', 'and', 'but', 'for', 'with', 'how', 'what', 'when'].includes(word))
    ?.slice(0, 5) || [];
  
  const complexity = query.length > 100 ? 'high' : query.split(' ').length > 10 ? 'medium' : 'low';
...
function evaluateEvidence(
  ...
  const avgQuality = chunks.reduce((sum, chunk) => sum + chunk.qualityScore, 0) / chunks.length;
...
async function addSourceAnalysis(
  ...
  const sourceAnalysis = {
    ...
    qualityDistribution: {
      ...
      medium: results.filter(r => r.qualityScore > 0.4 && r.qualityScore <= 0.7).length,
  ...
  for (const [sourceName, sourceResults] of Object.entries(sourceGroups)) {
    ...
    const avgQuality = resultsArray.reduce((sum, r) => sum + r.qualityScore, 0) / resultsArray.length;
  ...
  if (sourceAnalysis.qualityDistribution.low > sourceAnalysis.qualityDistribution.high) {
    sourceAnalysis.recommendations.push('Consider refining your query for more precise results');
  }
  
  if (sourceAnalysis.totalSources === 1) {
    sourceAnalysis.recommendations.push('Response based on single source - consider uploading additional relevant documents');
  }
  
  if (results.every(r => r.qualityScore < 0.5)) {
    sourceAnalysis.recommendations.push('Low relevance scores detected - the available documents may not fully address your question');
  }
