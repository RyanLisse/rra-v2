  ...
  async executeWithRetry<T>(
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        console.warn(`${context} failed (attempt ${attempt + 1}/${this.maxRetries + 1}), retrying in ${delay}ms:`, error);
        await new Promise(resolve => setTimeout(resolve, delay));
      }
    }
    
    throw new Error(`${context} failed after ${this.maxRetries + 1} attempts. Last error: ${lastError.message}`);
  }
}
...
export class EnhancedNeonApiClient {
  ...
  constructor(config: NeonApiClientConfig = {}) {
    ...
    if (!envCheck.valid) {
      console.warn(`Missing required environment variables: ${envCheck.missing.join(', ')}`);
    }

    this.config = {
      defaultProjectId: config.defaultProjectId || process.env.NEON_PROJECT_ID || 'yellow-tooth-01830141',
   ...
   */
  async getProject(projectId?: string): Promise<DatabaseOperationResult<NeonProject>> {
    const id = projectId || this.config.defaultProjectId;
    
    return this.executeOperation('get_project', async () => {
      await this.rateLimiter.checkLimit();
   ...
   */
  async createTestBranch(options: BranchCreationOptions): Promise<DatabaseOperationResult<TestBranchInfo>> {
    const {
      testSuite,
      purpose = 'testing',
      tags = [],
    ...
    return this.executeOperation('create_test_branch', async () => {
      await this.rateLimiter.checkLimit();
      ...
      const result = await this.retryManager.executeWithRetry(async () => {
        ...
        const branchInfo: TestBranchInfo = {
          ...
          database,
          role,
          host: this.extractHostFromConnectionString(connectionResponse.connection_string),
   ...
   */
  async deleteTestBranch(branchNameOrId: string): Promise<DatabaseOperationResult<void>> {
    return this.executeOperation('delete_test_branch', async () => {
      await this.rateLimiter.checkLimit();
   ...
   */
  async listBranches(projectId?: string): Promise<DatabaseOperationResult<NeonBranch[]>> {
    const id = projectId || this.config.defaultProjectId;
    
    return this.executeOperation('list_branches', async () => {
      await this.rateLimiter.checkLimit();
  ...
  async executeSql(
    ...
    return this.executeOperation('execute_sql', async () => {
      await this.rateLimiter.checkLimit();
  ...
  async executeTransaction(
    ...
    return this.executeOperation('execute_transaction', async () => {
      await this.rateLimiter.checkLimit();
  ...
  async getConnectionString(
    ...
    return this.executeOperation('get_connection_string', async () => {
      await this.rateLimiter.checkLimit();
   ...
   */
  async cleanupTestBranches(filters: CleanupFilters = {}): Promise<DatabaseOperationResult<{
    deleted: string[];
    failed: string[];
    skipped: string[];
  }>> {
   ...
   */
  async getBranchStatistics(projectId?: string): Promise<DatabaseOperationResult<{
    ...
    return this.executeOperation('get_branch_statistics', async () => {
      ...
      if (testBranches.length > 0) {
        const sorted = [...testBranches].sort((a, b) => 
      ...
      return {
        ...
        total_size_bytes: branches.reduce((sum, b) => sum + (b.logical_size || 0), 0),
  ...
  async withTestBranch<T>(
    ...
    try {
      ...
      await this.deleteTestBranch(branchInfo.branchName).catch(error =>
        console.error(`Failed to cleanup test branch ${branchInfo.branchName}:`, error)
      );
    }
  }
  ...
  private async executeOperation<T>(
    ...
    const operationId = this.logger.startOperation(operation, `Executing ${operation}`, metadata);
    const startTime = Date.now();
    ...
    try {
      ...
      // Log success
      this.logger.completeOperation(operationId, operation, true, duration, 'Operation completed successfully', {
        ...metadata,
      ...
      // Log error
      this.logger.completeOperation(operationId, operation, false, duration, errorMessage, {
        ...metadata,
 ...
 */
export function getNeonApiClient(config?: NeonApiClientConfig): EnhancedNeonApiClient {
  if (!globalNeonClient) {
