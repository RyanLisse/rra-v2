  ...
  try {
    ...
    if (projectsResult.success) {
      ...
      projectsResult.data?.forEach((project) => {
        console.log(
          `  - ${project.name} (${project.id}) - ${project.platform_id}/${project.region_id}`,
        );
      });
    }
    ...
    if (branchesResult.success) {
      ...
      branchesResult.data?.forEach((branch) => {
        console.log(
          `  - ${branch.name} (${branch.id}) - ${branch.current_state} ${branch.primary ? '[PRIMARY]' : ''}`,
        );
      });
    }
  } catch (error) {
    ...
    const testResult = await client.withTestBranch(
      ...
      async (branchInfo) => {
        ...
        if (!createTableResult.success) {
          throw new Error(
            `Failed to create test tables: ${createTableResult.error}`,
          );
        }
...
export async function monitoringExample() {
  ...
  try {
    ...
    recentLogs.forEach((log) => {
      ...
      console.log(
        `  ${log.timestamp} [${log.level.toUpperCase()}] ${log.operation}: ${log.message}${duration}`,
      );
    });
    ...
    await client.withTestBranch(
      ...
      async (branchInfo) => {
        ...
        if (!initialSchema.success) {
          throw new Error(
            `Initial schema creation failed: ${initialSchema.error}`,
          );
        }
  ...
  try {
    ...
    const parallelTests = testSuites.map(async (suiteName) => {
      return client.withTestBranch(
        ...
        async (branchInfo) => {
          console.log(
            `🚀 Running ${suiteName} on branch: ${branchInfo.branchName}`,
          );
    ...
    console.log(
      `🎉 Parallel testing completed: ${successCount}/${testSuites.length} suites passed`,
    );
  } catch (error) {
