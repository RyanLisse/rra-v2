...
async function runDemo() {
  ...
  try {
    ...
    if (branchesResult.success) {
      ...
      branchesResult.data?.forEach((branch: any) => {
        console.log(`  - ${branch.name} (${branch.id}) ${branch.primary ? '[PRIMARY]' : ''}`);
      });
    ...
    if (metrics.length > 0) {
      ...
      metrics.forEach(metric => {
        ...
        console.log(`    🕐 Last Executed: ${new Date(metric.lastExecuted).toLocaleTimeString()}`);
      });
    } else {
    ...
    if (recentLogs.length > 0) {
      ...
      recentLogs.forEach((log, index) => {
        ...
        console.log(`  ${index + 1}. [${time}] ${log.operation}: ${log.message}${duration}`);
      });
    } else {
    ...
    console.log(`  ⚙️ Configuration: ${JSON.stringify(exportData.config, null, 2)}`);
    console.log(`  🕐 Export Time: ${new Date(exportData.exportedAt).toLocaleString()}`);
    console.log();
    ...
    results.forEach(result => {
      console.log(`  ${result.index}. ${result.success ? '✅' : '❌'} (${result.duration}ms)`);
    });
  ...
  if (finalMetrics.length > 0) {
    const avgDuration = finalMetrics.reduce((sum, m) => sum + m.avgDuration, 0) / finalMetrics.length;
    const totalSuccessRate = finalMetrics.reduce((sum, m) => sum + m.successRate, 0) / finalMetrics.length;
