...
export class AdeClient {
  ...
  constructor(config: Partial<AdeConfig>) {
    ...
    const validatedConfig = AdeConfigSchema.parse({
      ...
      endpoint: config.endpoint || process.env.LANDING_AI_ENDPOINT || 'https://api.landing.ai/v1/ade',
   ...
   */
  async processDocument(filePath: string, documentId: string): Promise<LandingAiApiResponse> {
    try {
   ...
   */
  async simulateProcessing(filePath: string, documentId: string): Promise<LandingAiApiResponse> {
  ...
  private async validateAndReadFile(filePath: string): Promise<Buffer> {
    try {
      ...
      if (!filePath.toLowerCase().endsWith('.pdf')) {
        throw new AdeValidationError('Only PDF files are currently supported for ADE processing');
      }
      ...
      throw new AdeValidationError(`Failed to read file: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async executeWithRetries<T>(operation: () => Promise<T>): Promise<T> {
    ...
    for (let attempt = 0; attempt <= this.config.retries; attempt++) {
      try {
        ...
        if (error instanceof AdeValidationError || error instanceof AdeRateLimitError) {
          throw error;
        }
    ...
    for (let page = 1; page <= pageCount; page++) {
      ...
      if (Math.random() > 0.6) {
        ...
        elements.push({
          ...
          text_content: isTable ? 'Column 1\tColumn 2\tColumn 3\nValue 1\tValue 2\tValue 3' : undefined,
          image_data: !isTable ? `data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==` : undefined,
          page_number: page,
