...
const tests = [
  {
    ...
    test: () => {
      const result = execSync('curl -s -I http://localhost:3000/', { encoding: 'utf8' });
  ...
  },
  {
    ...
    test: () => {
      const result = execSync('curl -s -I http://localhost:3000/api/auth/login', { encoding: 'utf8' });
      const hasRedirect = result.includes('307') || result.includes('302');
      return hasRedirect;
    }
  },
  {
    ...
    test: () => {
      const result = execSync('curl -s -I http://localhost:3000/api/auth/guest', { encoding: 'utf8' });
      const hasRedirect = result.includes('307') || result.includes('302');
      return hasRedirect;
    }
  },
  {
    ...
    test: () => {
      try {
        const result = execSync(`curl -s -w "%{http_code}" -o /dev/null -X POST http://localhost:3000/api/chat -H "Content-Type: application/json" -d '{"id":"test","message":{"role":"user","content":"test"},"selectedChatModel":"gpt-4o-mini","selectedVisibilityType":"private"}'`, { encoding: 'utf8' });
  ...
  },
  {
    ...
    test: () => {
      try {
        const result = execSync('curl -s -w "%{http_code}" -o /dev/null -X POST http://localhost:3000/api/documents/upload', { encoding: 'utf8' });
        return result.trim() === '307'; // Should redirect to auth
      } catch (error) {
        return false;
      }
  ...
  },
  {
    ...
    test: () => {
      try {
        const result = execSync('curl -s -w "%{http_code}" -o /dev/null -X GET http://localhost:3000/api/search?q=test', { encoding: 'utf8' });
        return result.trim() === '307'; // Should redirect to auth
      } catch (error) {
        return false;
      }
  ...
  },
  {
    ...
    test: () => {
      try {
        const result = execSync('curl -s -w "%{http_code}" -o /dev/null -X GET http://localhost:3000/api/documents/list', { encoding: 'utf8' });
        return result.trim() === '307'; // Should redirect to auth
      } catch (error) {
        return false;
      }
  ...
  },
  {
    ...
    test: () => {
      try {
        const result = execSync('curl -s -w "%{http_code}" -o /dev/null http://localhost:3000/api/health', { encoding: 'utf8' });
  ...
  },
  {
    ...
    test: () => {
      try {
        const result = execSync('curl -s -w "%{http_code}" -o /dev/null http://localhost:3000/api/ping', { encoding: 'utf8' });
        return result.trim() === '200'; // Should be accessible
      } catch (error) {
        return false;
      }
