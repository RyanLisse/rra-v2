...
test.describe('Auth Flow Visual Documentation', () => {
  test('Capture auth flow journey with screenshots', async () => {
    ...
    try {
      ...
      await captureStep(
        'homepage-redirect',
        'Homepage redirects to auth when not logged in',
      );
      ...
      await captureStep(
        'kinde-oauth-page',
        'Redirected to Kinde OAuth login page',
      );
      ...
      await captureStep(
        ...
        'Documents route redirects to auth',
      );
  ...
  test('Monitor auth system performance', async ({ page }) => {
    ...
    for (let i = 0; i < metrics.totalRequests; i++) {
      ...
      try {
        ...
        console.log(
          `Request ${i + 1}: ${responseTime}ms - Status ${response?.status()}`,
        );
      } catch (error) {
        metrics.errorResponses++;
    ...
    const avgResponseTime =
      metrics.redirectTimes.reduce((a, b) => a + b, 0) /
      metrics.redirectTimes.length;
