...
import {
  setupSlice17TestContext,
  slice17Assertions,
} from '../config/slice-17-test-config';
...
test.describe('Slice 17: End-to-End Enhanced RAG Pipeline', () => {
  ...
  test('Complete document processing pipeline with ADE enhancement', async ({
    page,
  }) => {
    ...
    const { document, adeOutput, chunks } =
      await testContext.factories.createDocumentWithAde(user.id, 'complex');
    ...
    console.log(
      '✓ Document processing with ADE enhancement completed successfully',
    );
  ...
  test('Enhanced search functionality with structural metadata filtering', async ({
    page,
  }) => {
    // Create user and mixed document set
    const { user } = await testContext.factories.createUserWithAuth();
    const { enhancedDocs, legacyDocs, allChunks } =
      await testContext.factories.createMixedDocumentSet(user.id);

    // Test 1: Element type filtering
    const titleChunks = await testContext.utils.runEnhancedSearch(
      'system overview',
      {
        elementTypes: ['title'],
      },
    ...
    const page1To3Chunks = await testContext.utils.runEnhancedSearch(
      'configuration',
      {
    ...
    const structuredContent = await testContext.utils.runEnhancedSearch(
      'procedures',
      {
    ...
    const structuredTypes = [
      ...new Set(structuredContent.map((c) => c.elementType).filter(Boolean)),
    ];
    expect(structuredTypes.length).toBeGreaterThan(1);

    // Test 4: Spatial filtering
    const topAreaContent = await testContext.utils.runEnhancedSearch(
      'header information',
      {
  ...
  test('Context assembly with structural information for LLM prompts', async ({
    page,
  }) => {
    // Create user and test document
    const { user } = await testContext.factories.createUserWithAuth();
    const { document, chunks } =
      await testContext.factories.createDocumentWithAde(user.id, 'table_heavy');

    // Search for relevant content
    const relevantChunks = await testContext.utils.runEnhancedSearch(
    ...
    slice17Assertions.assertContextFormatting(enhancedContext, [
      'title',
      'paragraph',
      'table',
    ]);
    ...
    const safetyContext = testContext.utils.formatContextForLLM(
      relevantChunks.filter(
        (c) =>
          c.content.toLowerCase().includes('warning') ||
          c.content.toLowerCase().includes('safety'),
      ),
  ...
  test('Enhanced citation display with element types and page numbers', async ({
    page,
  }) => {
    ...
    const [document] = await testContext.db
      .insert(testContext.db.schema.ragDocument)
      .values(docData)
      .returning();
    ...
    const chunksWithDocument =
      await testContext.db.query.documentChunk.findMany({
    ...
    const legacyChunkWithDoc =
      await testContext.db.query.documentChunk.findFirst({
        where: (chunk, { eq }) => eq(chunk.id, legacyChunk.id),
        with: { document: true },
      });

    const legacyCitations = testContext.utils.generateCitations([
      legacyChunkWithDoc!,
    ]);
    ...
    console.log(
      `  - Element types in citations: ${[...new Set(citations.map((c) => c.elementType).filter(Boolean))].join(', ')}`,
    );
    console.log(`  - Legacy citation fallback tested`);
  });

  test('Performance validation: Enhanced vs legacy pipeline', async ({
    page,
  }) => {
    const { user } = await testContext.factories.createUserWithAuth();

    // Create performance test datasets
    const enhancedPerf = await testContext.utils.measureSearchPerformance(
      async () => {
        const { documents, chunks } =
          await testContext.factories.createPerformanceDataset(
            user.id,
            'medium',
          );
    ...
    const legacyPerf = await testContext.utils.measureSearchPerformance(
      async () => {
    ...
    if (enhancedResultCount > 0 && legacyResultCount > 0) {
      ...
      const enhancedWithMetadata = enhancedPerf.result.filter(
        (r) => r.elementType || r.pageNumber,
      );
      expect(enhancedWithMetadata.length).toBeGreaterThan(0);

      // Legacy results should not have metadata
      const legacyWithMetadata = legacyPerf.result.filter(
        (r) => r.elementType || r.pageNumber,
      );
    ...
    console.log(
      `    - Memory: ${(enhancedPerf.memoryUsage.heapUsed / 1024 / 1024).toFixed(2)}MB`,
    );
    console.log(`  Legacy Pipeline:`);
    console.log(`    - Duration: ${legacyPerf.duration}ms`);
    console.log(`    - Results: ${legacyResultCount} (no metadata)`);
    console.log(
      `    - Memory: ${(legacyPerf.memoryUsage.heapUsed / 1024 / 1024).toFixed(2)}MB`,
    );
  });
  ...
  test('LLM integration with enhanced system prompts', async ({ page }) => {
    ...
    const { document, chunks } =
      await testContext.factories.createDocumentWithAde(user.id, 'mixed');
    ...
    for (const scenario of testScenarios) {
      ...
      if (relevantChunks.length > 0) {
        ...
        console.log(
          `✓ LLM prompt for "${scenario.query}" (${scenario.contextType}):`,
        );
        console.log(`    - Relevant chunks: ${relevantChunks.length}`);
        console.log(`    - Prompt length: ${enhancedPrompt.length} chars`);
        console.log(
          `    - Element types: ${[...new Set(relevantChunks.map((c) => c.elementType).filter(Boolean))].join(', ')}`,
        );
      }
    }
    ...
    const mixedResults =
      await testContext.utils.runEnhancedSearch('system information');
  ...
  test('Complete workflow: Upload → Process → Search → Context → Citations', async ({
    page,
  }) => {
    ...
    expect(enhancedContext).toContain(
      'calibration and configuration procedures',
    );
    slice17Assertions.assertContextFormatting(enhancedContext, [
      'title',
    ...
    console.log(`  - Citations generated: ${citations.length}`);
    console.log(
      `  - Enhanced chunks: ${enhancedChunks.length}/${documentChunks.length}`,
    );
  });
});
