...
export class PerformanceSeeder extends BaseSeeder {
   ...
   */
  private async createPerformanceDatasets(rowsCreated: Record<string, number>): Promise<void> {
    const scale = this.getPerformanceScale();
    const scenarios = this.config.scenarios || ['users', 'documents', 'chats'];

    console.log(`Creating performance dataset at ${scale} scale with scenarios: ${scenarios.join(', ')}`);
    ...
    const result = await performanceFactory.createPerformanceDataset(scale, options);
    
    // Extract and insert the generated data
    await this.insertPerformanceData(result.dataset, rowsCreated);

    console.log(`✓ Performance dataset created with ${result.metrics.rowCount} total rows`);
  }
  ...
  private logPerformanceMetrics(): void {
    ...
    const byTable = metrics.reduce((acc, metric) => {
    ...
    Object.entries(byTable).forEach(([tableName, stats]) => {
      ...
      console.log(`  Rows/sec: ${Math.round(stats.totalRows / (stats.totalTime / 1000))}`);
      console.log('');
    });
    ...
    console.log(`  Total Time: ${totalMetrics.time}ms (${Math.round(totalMetrics.time / 1000)}s)`);
    console.log(`  Peak Memory: ${Math.round(totalMetrics.memory / 1024 / 1024)}MB`);
    console.log(`  Overall Rate: ${Math.round(totalMetrics.rows / (totalMetrics.time / 1000))} rows/sec`);
  }
}
...
export class LoadTestSeeder extends BaseSeeder {
   ...
   */
  private async createLoadTestScenarios(rowsCreated: Record<string, number>): Promise<void> {
  ...
  private async insertScenarioData(
    ...
    const scenarioMetrics = this.getMetrics().filter(m => 
      m.timestamp.getTime() > Date.now() - 60000 // Last minute
    );
    
    console.log(`  📊 Scenario ${scenarioName}: ${scenarioMetrics.length} operations`);
  }
  ...
  private async insertPerformanceData(
    ...
    const insertMethod = (performanceSeeder as any).insertPerformanceData.bind(performanceSeeder);
...
export class StressTestSeeder extends BaseSeeder {
   ...
   * Create stress testing scenarios
   */
  private async createStressTestScenarios(rowsCreated: Record<string, number>): Promise<void> {
    ...
    for (const scenario of scenarios) {
      if (this.shouldRunStressScenario(scenario.name)) {
        ...
        try {
          ...
          console.log(`  Time: ${executionTime}ms, Memory: ${Math.round(memoryUsed / 1024 / 1024)}MB`);
          
          // Check if we exceeded expected metrics
          if (executionTime > scenario.metrics.maxSetupTime) {
            console.warn(`⚠️  Scenario exceeded expected setup time: ${executionTime}ms > ${scenario.metrics.maxSetupTime}ms`);
          }
          
          if (memoryUsed > scenario.metrics.maxMemoryUsage) {
            console.warn(`⚠️  Scenario exceeded expected memory usage: ${Math.round(memoryUsed / 1024 / 1024)}MB > ${Math.round(scenario.metrics.maxMemoryUsage / 1024 / 1024)}MB`);
          }
  ...
  private async insertStressData(
    ...
    this.batchInsert = async <T>(table: any, data: T[], batchSize: number = 500) => {
      return originalBatchInsert.call(this, table, data, Math.min(batchSize, 500));
    };
    ...
    if (data.documents) {
      await this.batchInsert(schema.ragDocument, data.documents.map((d: any) => d.document), 200);
      await this.batchInsert(schema.documentContent, data.documents.map((d: any) => d.content), 200);
      ...
      rowsCreated.ragDocuments = (rowsCreated.ragDocuments || 0) + data.documents.length;
      rowsCreated.documentContent = (rowsCreated.documentContent || 0) + data.documents.length;
      rowsCreated.documentChunks = (rowsCreated.documentChunks || 0) + chunks.length;
      rowsCreated.documentEmbeddings = (rowsCreated.documentEmbeddings || 0) + embeddings.length;
    }

    if (data.extraEmbeddings) {
      await this.batchInsert(schema.documentEmbedding, data.extraEmbeddings, 100);
      rowsCreated.documentEmbeddings = (rowsCreated.documentEmbeddings || 0) + data.extraEmbeddings.length;
    }

    if (data.concurrentSessions) {
      await this.batchInsert(schema.session, data.concurrentSessions, 1000);
      rowsCreated.sessions = (rowsCreated.sessions || 0) + data.concurrentSessions.length;
    }
