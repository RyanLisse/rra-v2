...
describe('Vector Search Performance Tests (Enhanced with Neon Branching)', () => {
  ...
  describe('Embedding Storage Performance', () => {
    it(
      'should efficiently store large batches of embeddings',
      async () => {
        const testStart = Date.now();
        logger.info(
          'performance_test',
          'Starting large batch embedding storage test',
        );
        ...
        const chunks = performanceFactory.createBulkDocumentChunks(
          document.id,
          chunkCount,
        );
        ...
        const embeddings = insertedChunks.map((chunk, index) => ({
          ...
          embedding: JSON.stringify(
            performanceFactory.createRealisticEmbedding(1536),
        ...
        logger.info(
          'performance_test',
          'Large batch embedding storage test completed',
          {
    ...
    it(
      'should handle concurrent embedding operations at scale',
      async () => {
        const testStart = Date.now();
        logger.info(
          'performance_test',
          'Starting concurrent embedding operations test',
        );
        ...
        logger.info(
          'performance_metrics',
          'Documents created for concurrent test',
          {
            docCount,
            chunksPerDoc,
        ...
        logger.info(
          'performance_metrics',
          'Concurrent chunk insertion completed',
          {
        ...
        const { duration: embeddingTime, memoryUsage } =
          await measurePerformance(async () => {
        ...
        logger.info(
          'performance_test',
          'Concurrent embedding operations test completed',
          {
            totalTime: Date.now() - testStart,
  ...
  describe('Vector Search Performance', () => {
    it(
      'should perform fast similarity searches with large datasets',
      async () => {
        const testStart = Date.now();
        logger.info(
          'performance_test',
          'Starting large dataset vector search test',
        );
        ...
        logger.info(
          'performance_metrics',
          'Creating large dataset for vector search',
          {
            chunkCount,
        ...
        logger.info(
          'performance_metrics',
          'Creating embeddings with search patterns',
        );
        ...
        for (let i = 0; i < allInsertedChunks.length; i += embeddingBatchSize) {
          ...
          const embeddings = chunkBatch.map((chunk, index) => ({
            chunkId: chunk.id,
            embedding: JSON.stringify(
              performanceFactory.createSearchableEmbedding(1536, index % 10),
        ...
        logger.info(
          'performance_metrics',
          'Dataset creation completed, starting search tests',
        );
        ...
        const averageSearchTime =
          searchTimes.reduce((sum, time) => sum + time, 0) / searchQueries;
        const maxSearchTime = Math.max(...searchTimes);
        const minSearchTime = Math.min(...searchTimes);
        const searchTimeVariance =
          searchTimes.reduce(
            (sum, time) => sum + Math.pow(time - averageSearchTime, 2),
            0,
          ) / searchQueries;
        ...
        expect(searchTimeVariance).toBeLessThan(
          Math.pow(expectedAvgSearchTime * 0.5, 2),
        ...
        logger.info(
          'performance_test',
          'Large dataset vector search test completed',
          {
            totalTime: Date.now() - testStart,
    ...
    it(
      'should demonstrate excellent scaling characteristics across dataset sizes',
      async () => {
        const testStart = Date.now();
        logger.info(
          'performance_test',
          'Starting search scaling analysis test',
        );
        ...
        for (const size of datasetSizes) {
          ...
          const avgSearchTime =
            searchTimes.reduce((sum, time) => sum + time, 0) /
            searchTimes.length;
          const maxSearchTime = Math.max(...searchTimes);
          const minSearchTime = Math.min(...searchTimes);
          ...
          performanceResults.push({
            ...
            searchVariance:
              searchTimes.reduce(
                (sum, time) => sum + Math.pow(time - avgSearchTime, 2),
                0,
              ) / searchTimes.length,
        ...
        const avgScalingFactor =
          scalingFactors.reduce((sum, factor) => sum + factor, 0) /
          scalingFactors.length;
        const maxSearchTimeOverall = Math.max(
          ...performanceResults.map((r) => r.maxSearchTime),
        );
        const searchTimeGrowthRate =
          performanceResults[performanceResults.length - 1].avgSearchTime /
          performanceResults[0].avgSearchTime;
        ...
        performanceResults.forEach((result, index) => {
          ...
          expect(result.searchVariance).toBeLessThan(
            Math.pow(result.avgSearchTime * 0.3, 2),
          ); // Consistent performance
        });
  ...
  describe('Memory Usage Optimization', () => {
    it(
      'should efficiently handle various embedding dimensions at scale',
      async () => {
        const testStart = Date.now();
        logger.info(
          'performance_test',
          'Starting memory optimization test for embedding dimensions',
        );
        ...
        for (const dimension of embeddingDimensions) {
          ...
          const embeddings = insertedChunks.map((chunk, index) => ({
            chunkId: chunk.id,
            embedding: JSON.stringify(
              performanceFactory.createRealisticEmbedding(dimension),
            ),
            model: `test-model-${dimension}d`,
          }));

          const { duration, memoryUsage } = await measurePerformance(
            async () => {
          ...
          const avgEmbeddingSize = JSON.stringify(
            performanceFactory.createRealisticEmbedding(dimension),
          ).length;
          const theoreticalMemoryMB =
            (chunksPerDimension * avgEmbeddingSize) / (1024 * 1024);
          ...
          await db.delete(schema.documentChunk).where((chunk, { inArray }) =>
            inArray(
              chunk.id,
              insertedChunks.map((c) => c.id),
            ),
        ...
        const avgMemoryScaling =
          memoryScalingFactors.reduce((sum, factor) => sum + factor, 0) /
          memoryScalingFactors.length;
    ...
    it(
      'should efficiently stream and process massive result sets',
      async () => {
        const testStart = Date.now();
        logger.info(
          'performance_test',
          'Starting massive result set streaming test',
        );
        ...
        logger.info(
          'performance_metrics',
          'Creating massive dataset for streaming test',
          {
            chunkCount,
        ...
        logger.info(
          'performance_metrics',
          'Massive dataset created, starting streaming test',
        );
        ...
        for (const pageSize of pageSizes) {
          ...
          const { duration, memoryUsage } = await measurePerformance(
            async () => {
              ...
              const memoryCheckInterval = Math.max(
                1,
                Math.floor(totalPages / 10),
              );
          ...
          logger.info(
            'performance_metrics',
            `Streaming test completed for page size ${pageSize}`,
            {
              duration,
              memoryUsageMB: memoryUsage.heapUsed / (1024 * 1024),
              throughput: totalProcessed / (duration / 1000),
        ...
        logger.info(
          'performance_test',
          'Massive result set streaming test completed',
          {
            totalTime: Date.now() - testStart,
            datasetSize: chunkCount,
  ...
  describe('Index Performance', () => {
    it(
      'should efficiently handle complex vector index operations',
      async () => {
        const testStart = Date.now();
        logger.info(
          'performance_test',
          'Starting complex vector index operations test',
        );
        ...
        const docsData = performanceFactory.createBulkDocuments(
          user.id,
          docCount,
        );
        ...
        logger.info(
          'performance_metrics',
          'Creating distributed dataset for index testing',
          {
            docCount,
        ...
        logger.info(
          'performance_metrics',
          'Generated chunks for index testing',
          {
            totalChunks,
            documentChunkCounts,
        ...
        const { duration: lookupTime, result: lookupResults } =
          await measurePerformance(async () => {
            const results = [];
        ...
        const { duration: crossDocSearchTime, result: crossDocResults } =
          await measurePerformance(async () => {
            const results = [];
            ...
            const subsetDocs = documents.slice(
              0,
        ...
        logger.info(
          'performance_test',
          'Complex vector index operations test completed',
          {
            totalTime: Date.now() - testStart,
            totalChunks,
            docCount,
    ...
    it(
      'should maintain performance under intensive update and query loads',
      async () => {
        const testStart = Date.now();
        logger.info(
          'performance_test',
          'Starting intensive update and query load test',
        );
        ...
        const initialChunks = performanceFactory.createBulkDocumentChunks(
          document.id,
          initialChunkCount,
        );
        ...
        logger.info(
          'performance_metrics',
          'Initial dataset created for update load test',
          {
        ...
        const { duration: intensiveOperationTime, memoryUsage } =
          await measurePerformance(async () => {
            for (let i = 0; i < operationCount; i++) {
              ...
              switch (operationType) {
                ...
                case 1: {
                  ...
                  await db
                    ...
                    .where(
                      schema.documentEmbedding.chunkId ===
                        randomChunkForEmbedding.id,
                    );
              ...
              if (i > 0 && i % 100 === 0) {
                ...
                if (
                  ...
                  logger.warn(
                    'performance_warning',
                    'Memory usage high during intensive operations',
                    {
        ...
        const { duration: finalSearchTime, result: finalResults } =
          await measurePerformance(async () => {
            return db.query.documentChunk.findMany({
              where: (chunk, { eq }) => eq(chunk.documentId, document.id),
              with: { embedding: true },
        ...
        logger.info(
          'performance_test',
          'Intensive update and query load test completed',
          {
            totalTime: Date.now() - testStart,
            operationCount,
  ...
  describe('Parallel Performance Benchmarks', () => {
    it(
      ...
      async () => {
        ...
        const { duration: writeTime, result: writeCounts } =
          await measurePerformance(async () => {
        ...
        const { duration: readTime, result: readResults } =
          await measurePerformance(async () => {
        ...
        logger.info(
          'performance_test',
          'Concurrent read/write benchmark completed',
          {
            totalTime: Date.now() - testStart,
            writeTime,
            readTime,
