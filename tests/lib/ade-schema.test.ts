...
describe('ADE Schema Changes', () => {
  ...
  it('should query chunks by element type', async () => {
    ...
    const foundTitleChunk = titleChunks.find(
      (chunk) => chunk.id === titleChunk[0].id,
    );
    ...
    await db
      .delete(documentChunk)
      .where(eq(documentChunk.id, titleChunk[0].id));
    await db
      .delete(documentChunk)
      .where(eq(documentChunk.id, paragraphChunk[0].id));
  });

  it('should query chunks by page number', async () => {
    // Create chunks on different pages
    const page1Chunk = await db
      .insert(documentChunk)
      .values({
        documentId: testDocumentId,
        chunkIndex: '20',
    ...
    const foundPage1Chunk = page1Chunks.find(
      (chunk) => chunk.id === page1Chunk[0].id,
    );
    ...
    await db
      .delete(documentChunk)
      .where(eq(documentChunk.id, page1Chunk[0].id));
    await db
      .delete(documentChunk)
      .where(eq(documentChunk.id, page2Chunk[0].id));
  });

  it('should handle bbox coordinates as JSONB', async () => {
    const bboxChunk = await db
      .insert(documentChunk)
      .values({
        documentId: testDocumentId,
        chunkIndex: '30',
    ...
    const complexBboxChunk = await db
      .insert(documentChunk)
      .values({
        documentId: testDocumentId,
        chunkIndex: '31',
    ...
    await db
      .delete(documentChunk)
      .where(eq(documentChunk.id, complexBboxChunk[0].id));
  });
