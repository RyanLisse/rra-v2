...
const tests = [
  {
    ...
    test: () => {
      const result = execSync(`curl -s -I "${tests[0].url}" | head -5`, { encoding: 'utf8' });
  ...
  },
  {
    ...
    test: () => {
      const result = execSync(`curl -s -I "${tests[1].url}" | head -5`, { encoding: 'utf8' });
      console.log('Response headers:\n', result);
  ...
  },
  {
    ...
    test: () => {
      const result = execSync(`curl -s -I "${tests[2].url}" | head -5`, { encoding: 'utf8' });
      console.log('Response headers:\n', result);
  ...
  },
  {
    ...
    test: () => {
      const result = execSync(`curl -s -I "${tests[3].url}" | head -5`, { encoding: 'utf8' });
      console.log('Response headers:\n', result);
      return result.includes('307') || result.includes('302');
    }
  },
  {
    ...
    test: () => {
      const result = execSync(`curl -s -w "%{http_code}" -o /dev/null -X POST "${tests[4].url}" -H "Content-Type: application/json" -d '{"id":"test","message":{"role":"user","content":"test"}}'`, { encoding: 'utf8' });
  ...
  },
  {
    ...
    test: () => {
      const result = execSync(`curl -s "${tests[5].url}"`, { encoding: 'utf8' });
...
try {
  ...
  for (let i = 0; i < 3; i++) {
    execSync('curl -s -o /dev/null http://localhost:3000/api/auth/login', { encoding: 'utf8' });
...
protectedRoutes.forEach(route => {
  try {
    const result = execSync(`curl -s -I "http://localhost:3000${route}" | grep -i location | head -1`, { encoding: 'utf8' });
