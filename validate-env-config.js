...
const kindeStatus = kindeVars.map(varName => {
  ...
  const isSet = !!value;
  const isValid = value && value.length > 0 && !value.includes('<') && !value.includes('>');
  
  console.log(`${isValid ? '✅' : '❌'} ${varName}: ${isSet ? 'Set' : 'Missing'} ${isValid ? '' : '(Invalid format)'}`);
...
const aiStatus = aiVars.map(varName => {
  ...
  console.log(`${isValid ? '✅' : '⚠️ '} ${varName}: ${isSet ? 'Set' : 'Missing'} ${isValid ? '' : '(Check format)'}`);
...
const dbStatus = dbVars.map(varName => {
  const value = envVars[varName];
  const isSet = !!value;
  const isValid = value && (value.startsWith('postgres://') || value.startsWith('postgresql://'));
...
console.log(`🔐 Kinde Auth: ${kindeComplete ? '✅ Complete' : '❌ Incomplete'}`);
console.log(`🤖 AI Providers: ${hasAiProvider ? '✅ At least one configured' : '❌ None configured'}`);
console.log(`🗄️  Database: ${dbComplete ? '✅ Configured' : '❌ Not configured'}`);
...
if (!overallHealthy) {
  ...
  if (!hasAiProvider) {
    console.log('- Configure at least one AI provider (OpenAI, Anthropic, or Gemini)');
  }
  if (!dbComplete) {
...
try {
  ...
  const hasExclusions = middlewareContent.includes('ping') || middlewareContent.includes('health');
  
  console.log(`✅ Kinde middleware: ${hasKindeMiddleware ? 'Enabled' : 'Missing'}`);
  console.log(`✅ Error handling: ${hasErrorHandling ? 'Configured' : 'Missing'}`);
  console.log(`✅ Endpoint exclusions: ${hasExclusions ? 'Configured' : 'Missing'}`);
} catch (error) {
